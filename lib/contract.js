  ...
  async init() {
    ...
    switch (network) {
      ...
      case "gnosis": {
        ...
        let httpweb3provider = new HttpProvider(process.env.GNOSIS_RPC_URL);
        console.log(httpweb3provider);
        provider = new ethers.providers.Web3Provider(httpweb3provider);
        account = new ethers.Wallet(process.env.PRIVATE_KEY, provider);
    ...
    const podContract = new ethers.Contract(PODcontracts[network], abi, signer);
